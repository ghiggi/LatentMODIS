...
# - Rename NDI
new_nm = [re.sub("_","-",re.sub("ch_","",s)) for s in df.columns]
df.columns = new_nm
...
corrMatrix = df.corr()
corrMatrix_VIS = df_VIS.corr()
corrMatrix_IR = df_IR.corr()
...
cg.ax_heatmap.set_xticklabels(xlabels, size=fontsize)
cg.ax_heatmap.set_yticklabels(ylabels, size=fontsize)

cg.ax_heatmap.set_title('Correlation between NDIs')
# - Add supertitle
# cg.fig.suptitle('Correlation between NDI')
 
# - Save figure
cg.fig.savefig(fname=os.path.join(figs_dir, "CorrMatrix_NDIs.png"))

#-----------------------------------------------------------------------------.
### Display clustered correlation matrix (VIS) 
fontsize = 7

sns.set(rc={'figure.figsize':(15,15),
            'figure.dpi': 300})
cg = sns.clustermap(corrMatrix_VIS, 
                    cbar_pos=(0, .25, .02, .4), # left, bottom, width, height
                    cbar_kws={'label': 'Correlation'},
                    dendrogram_ratio=(0.1,0.1),
                    cmap = sns.diverging_palette(20, 220, n=200),
...
cg.ax_heatmap.set_title('Correlation between VIS NDIs')
# - Add supertitle
# cg.fig.suptitle('Correlation between NDI')
 
# - Save figure
cg.fig.savefig(fname=os.path.join(figs_dir, "CorrMatrix_NDIs_VIS.png"))

#-----------------------------------------------------------------------------.
### Display clustered correlation matrix (IR) 
fontsize = 7

sns.set(rc={'figure.figsize':(15,15),
            'figure.dpi': 300})
cg = sns.clustermap(corrMatrix_IR, 
                    cbar_pos=(0, .25, .02, .4), # left, bottom, width, height
                    cbar_kws={'label': 'Correlation'},
                    dendrogram_ratio=(0.1,0.1),
                    cmap = sns.diverging_palette(20, 220, n=200),
...
cg.ax_heatmap.set_title('Correlation between IR NDIs')
# - Add supertitle
# cg.fig.suptitle('Correlation between NDI')
 
# - Save figure
cg.fig.savefig(fname=os.path.join(figs_dir, "CorrMatrix_NDIs_IR.png"))